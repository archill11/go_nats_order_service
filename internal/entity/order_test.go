...
func Test_OrderEntityGetters(t *testing.T) {
	...
	if ord.GetUID() != mockUID {
		t.Errorf("order UID %v not assert mock UID %v", ord.GetUID(), mockUID)
	}
	if ord.GetJsonData() != mockJsonData {
		t.Errorf("order JsonData %v not assert mock JsonData  %v", ord.GetJsonData(), mockJsonData)
	}
	if ord.GetUID() != *ord.GetUIDPtr() {
		t.Errorf("order UID %v not assert UID pointer %v ", ord.GetUID(), *ord.GetUIDPtr())
	}
	if ord.GetJsonData() != *ord.GetJsonDataPtr() {
		t.Errorf("order JsonData %v not assert JsonData pointer  %v", ord.GetUID(), *ord.GetJsonDataPtr())
	}
	...
	ord.SetUID(changedMockUID)
	if ord.GetUID() != changedMockUID {
		t.Errorf("%v not assert %v after set SetUID mrthod", ord.GetUID(), changedMockUID)
	}
	ord.SetJsonData(changedMockJsonData)
	if ord.GetJsonData() != changedMockJsonData {
		t.Errorf("order JsonData %v not assert mock JsonData  %v after set JsonData mrthod", ord.GetJsonData(), changedMockJsonData)
	}
}
